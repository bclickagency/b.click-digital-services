  ...
  return (
    <Layout>
      ...
      <PageHero
        ...
        title="نحن"
        highlight="B.CLICK"
        subtitle="شركة خدمات رقمية متكاملة نسعى لتحويل رؤية عملائنا إلى حقيقة رقمية مذهلة"
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <div className="space-y-4 text-muted-foreground leading-relaxed">
              <p>تأسست B.CLICK برؤية واضحة: أن نكون الشريك الرقمي الأمثل للشركات والأفراد الطموحين في مصر والوطن العربي.</p>
              <p>نؤمن بأن كل فكرة تستحق أن تُرى، وكل مشروع يستحق أن يُنفذ باحترافية. لذلك نجمع بين الإبداع والتقنية لنقدم حلولًا رقمية تتجاوز التوقعات.</p>
              <p>فريقنا من المتخصصين يعمل بشغف لتحقيق أهداف عملائنا، مع الالتزام بأعلى معايير الجودة والاحترافية.</p>
            </div>
          </motion.div>
          ...
          >
            <div className="glass-card text-center">
              ...
              <p className="text-sm text-muted-foreground">أن نكون الخيار الأول للحلول الرقمية في الوطن العربي</p>
            </div>
            <div className="glass-card text-center">
              <Target className="w-10 h-10 text-secondary mx-auto mb-3" />
              <h4 className="font-bold text-foreground mb-2">رسالتنا</h4>
              <p className="text-sm text-muted-foreground">تمكين الأعمال من النجاح في العالم الرقمي</p>
            </div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="relative max-w-3xl mx-auto">
          ...
          {timeline.map((item, index) => (
            <motion.div
              ...
              className={`relative flex items-center gap-6 mb-8 ${index % 2 === 0 ? 'lg:flex-row-reverse' : ''}`}
            >
      ...
      <section className="section-container">
        <div className="text-center mb-12">
          <motion.span initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="inline-block px-4 py-2 rounded-xl bg-secondary/10 text-secondary text-sm font-medium mb-4">
            لماذا B.CLICK؟
          </motion.span>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {whyUs.map((item, index) => (
            <motion.div key={item.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="glass-card text-center hover:scale-105 transition-transform duration-300">
      ...
      <section className="section-container bg-muted/30">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {values.map((value, index) => (
            <motion.div key={value.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="glass-card text-center">
              <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center mx-auto mb-4">
      ...
      <section className="section-container">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {team.map((member, index) => (
            <motion.div key={member.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="service-card text-center">
      ...
      {/* CTA */}
      <section className="section-container">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="glass-card text-center py-12 px-8 relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-primary/10 to-secondary/10" />
          <div className="relative z-10">
            <h2 className="section-title mb-4">هل أنت مستعد للعمل معنا؟</h2>
            <p className="section-subtitle mx-auto mb-8">تواصل معنا اليوم واحصل على استشارة مجانية</p>
            <div className="flex flex-wrap justify-center gap-4">
              <Link to="/request" className="btn-secondary">ابدأ مشروعك الآن <ArrowLeft className="w-5 h-5" /></Link>
